...
class sendsms
{
	 ...
	 */
	function __construct($apiurl="http://alerts.solutionsinfini.com/api/v3",$method="sms",$wk="A171b37139eaa6a916fd9f0184de1d9e9",$sd="WIWIGO")
	{
	 ...
	 * 
	 */
	function send_sms($to,$message,$type="json")
	{
		return $this->process_sms($to,$message,$type="json",$time="null",$unicode="null");
	}
	 ...
	 * 
	 */
	function schedule_sms($to,$message,$type="json",$time)
	{ 
		$this->process_sms($to,$message,$type="json",$time,$unicode='');
	}

	/**
	 * function to send unicode message
	 */
	function unicode_sms($to,$message,$type="json",$unicode)
	{  
		$this->process_sms($to,$message,$type="json",$time='',$unicode);
	}
	 ...
	 */
	function process_sms($to,$message,$type="json",$time='',$unicode='')
	{  
	...
	if($unicode=='null')
		...
		$unicode="&unicode=$unicode";
	 	...
	 	$url="$this->start$this->api_url/index.php?method=$this->api_method&api_key=$this->working_key&sender=$this->sender_id&to=$to&message=$message&format=$type$time$unicode";
	...
	{
		$url="$this->start$this->api_url/index.php?method=sms.status&api_key=$this->working_key&id=$mid&format=json";
			$this->execute($url);
	...
	{
		 $url="$this->start$this->api_url/index.php?method=sms.groupstatus&api_key=$this->working_key&groupid=$gid&format=json";
		$this->execute($url);
	...
	function execute($url)
	{
		...
		$output=curl_exec($ch);
		curl_close($ch);
		return json_decode($output)->status;
	}    
}
?>
